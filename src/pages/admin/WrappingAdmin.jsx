...
const WrappingAdmin = () => {
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on('postgres_changes', { 
        ...
        filter: 'source=eq.wrapping_page'
      }, () => {
        fetchRequests();
      })
      .subscribe();
  ...
  const fetchRequests = async () => {
    try {
      ...
      console.error('Error fetching wrapping requests:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${
        serviceType === 'wrapping'
          ? 'bg-gt-gold/20 text-gt-gold border-gt-gold/30'
          : 'bg-blue-500/20 text-blue-500 border-blue-500/30'
      }`}>
        {serviceType === 'wrapping' ? '🎨 WRAPPING' : '🛡️ PPF'}
      </span>
    );
  };
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-400">
                      <p>📧 {request.email}</p>
                      <p>📱 {request.phone}</p>
                      <p>🌐 {request.language === 'es' ? 'Español' : 'English'}</p>
                    </div>
